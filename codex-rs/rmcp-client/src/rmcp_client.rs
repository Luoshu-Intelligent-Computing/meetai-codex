...
/// Binds the progress token generated by RMCP to the caller's active request.
pub type BindProgressToken = Arc<dyn Fn(String) + Send + Sync>;
...
impl RmcpClient {
    ...
    pub async fn initialize(
        ...
        self.initialize_with_progress(params, timeout, send_elicitation, None)
            .await
    }

    /// Perform the initialization handshake and forward MCP progress notifications.
    #[instrument(level = "trace", skip_all)]
    ...
    pub async fn call_tool_with_progress(
        ...
        meta: Option<serde_json::Value>,
        timeout: Option<Duration>,
        bind_progress_token: Option<BindProgressToken>,
    ) -> Result<CallToolResult> {
        self.refresh_oauth_if_needed().await?;
        ...
        let result = self
            .run_service_operation("tools/call", timeout, move |service| {
                ...
                let meta = meta.clone();
                let bind_progress_token = bind_progress_token.clone();
                async move {
                    let modern_session = requested_modern
                        && service.peer().peer_info().is_some_and(|info| {
                            info.protocol_version == ProtocolVersion::V_2026_07_28
                    ...
                    let handle = service
                        ...
                        .await?;
                    if let Some(bind_progress_token) = bind_progress_token {
                        bind_progress_token(handle.progress_token.0.to_string());
                    }
